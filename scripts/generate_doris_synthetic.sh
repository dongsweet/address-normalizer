...
GENERATOR="${ROOT_DIR}/scripts/generate_doris_synthetic.py"
...
if command -v docker >/dev/null 2>&1 \
  ...
  exec docker run --rm \
    ...
    python /work/scripts/generate_doris_synthetic.py "$@"
fi

cat >&2 <<'EOF'
Could not find a usable Doris synthetic-data runtime.
